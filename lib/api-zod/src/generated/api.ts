...
export const SunoGenerateBody = zod.object({
  styleOfMusic: zod.string().max(1000).describe("Style tags for the Suno style field (max 1000 chars)"),
  lyrics: zod.string().max(5000).describe("Lyrics / metadata block (max 5000 chars)"),
  title: zod.string().max(80).describe("Song title (max 80 chars)"),
  negativePrompt: zod.string().max(200).optional().describe("Negative prompt (max 200 chars)"),
  instrumental: zod.boolean().optional().describe("Generate as instrumental (no lyrics)"),
  model: zod.string().optional().describe("Suno model version, e.g. V5_5"),
});

export const SunoTrackSchema = zod.object({
  id: zod.string().describe("Suno track ID"),
  title: zod.string().optional(),
  audio_url: zod.string().optional().describe("Direct audio URL"),
  image_url: zod.string().optional().describe("Cover art URL"),
...
export const SunoGenerateResponse = zod.object({
  taskId: zod.string().describe("Task ID for polling"),
  status: zod.enum(["queued", "processing", "completed", "failed"]).describe("Initial task status"),
  tracks: zod.array(SunoTrackSchema).optional().describe("Tracks if instantly available"),
});

export const SunoStatusResponse = zod.object({
  taskId: zod.string(),
  status: zod.enum(["queued", "processing", "completed", "failed"]),
  tracks: zod.array(SunoTrackSchema).optional(),
  error: zod.string().optional(),
});

export const SunoCreditsResponse = zod.object({
  credits: zod.number().describe("Remaining credits on sunoapi.org"),
});
