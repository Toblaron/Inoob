...
const KNOWN_EXCLUSION_VALUES = new Set([
  ...QUALITY_EXCLUSIONS.map((item) => item.value),
  ...ELEMENT_EXCLUSIONS.flatMap((item) => item.value.split(",")),
]);
...
/* ─── Runtime-validated autofill response parser ──────────────────────────────
 * The /api/autofill-from-url endpoint goes through an LLM which occasionally
 * returns malformed shapes (strings where arrays are expected, out-of-range
 * numbers, unknown enum values). Previously we trusted the backend contract
 * and destructured `raw.* ?? defaults` which silently accepted broken data
 * and blew up downstream in .slice()/.join() calls. This parser is the single
 * source of truth for "did this field come back valid enough to use?".
 */
const VALID_ENERGY = new Set(["very chill", "chill", "medium", "high", "intense"]);
const VALID_ERA = new Set(["50s", "60s", "70s", "80s", "90s", "2000s", "2010s", "modern"]);
const VALID_TEMPO = new Set(["ballad", "slow", "mid", "groove", "uptempo", "fast", "hyper"]);
const VALID_VOCALS = new Set(["male", "female", "mixed", "duet", "no vocals"]);

/** Coerce to string if primitive-stringy, trim, return null if empty/garbage. */
...
/** Coerce to a clean array of trimmed, non-empty strings. Accepts either
 *  `string[]` or a comma-separated string (LLMs occasionally flatten). */
function asStringArray(v: unknown, maxItems: number, maxLen = 120): string[] {
  const source: unknown[] = Array.isArray(v)
    ? v
    : typeof v === "string"
      ? v.split(",")
      : [];
...
/** Parse number-ish input (accepts "171" as well as 171). Returns null if
 *  not finite or out of [min, max]. */
...
/** Narrow to a known enum value, else null. */
function asEnum(v: unknown, valid: Set<string>): string | null {
...
/** Full parse of the /api/autofill-from-url response body. Never throws,
 *  always returns a fully-shaped SuggestedControls with safe defaults. */
function parseAutofillResponse(raw: unknown): SuggestedControls {
  const r = (raw && typeof raw === "object" ? raw : {}) as Record<string, unknown>;
  return {
    genres: asStringArray(r.genres, 8),
    era: asEnum(r.era, VALID_ERA),
    energy: asEnum(r.energy, VALID_ENERGY),
    tempo: asEnum(r.tempo, VALID_TEMPO),
    vocals: asEnum(r.vocals, VALID_VOCALS),
    moods: asStringArray(r.moods, 8),
    instruments: asStringArray(r.instruments, 8),
    genreNudge: asString(r.genreNudge, 240) ?? asString(r.vibeDescription, 240),
    bpm: asNumberInRange(r.bpm, 40, 300),
    key: asString(r.key, 40),
    chordProgression: asString(r.chordProgression, 200),
    vocalPersona: asString(r.vocalPersona, 400),
    sonicDna: asString(r.sonicDna, 400),
    metaTags: asStringArray(r.metaTags, 20, 40),
    pronunciationGuide: asString(r.pronunciationGuide, 240),
    negativeHints: asStringArray(r.negativeHints, 10, 60),
    vibeDescription: asString(r.vibeDescription, 400),
    weirdness: asNumberInRange(r.weirdness, 0, 100),
    styleInfluence: asNumberInRange(r.styleInfluence, 0, 100),
    audioInfluence: asNumberInRange(r.audioInfluence, 0, 100),
    songTitle: asString(r.songTitle, 200) ?? "",
    artist: asString(r.artist, 200) ?? "",
    mbTags: asStringArray(r.mbTags, 20, 60),
  };
}
...
export default function Home() {
  ...
  const moodBoardResult = useMemo<MoodBoardResult | null>(() => {
    ...
    return {
      ...
      moods: suggestions.moods,
      energy: suggestions.energy ?? "medium",
      tempo: suggestions.tempo ?? "mid",
      era: suggestions.era ?? "modern",
      instruments: suggestions.instruments,
      primaryGenre: suggestions.genres[0] ?? "Detected Song Vibe",
      reasoning: suggestions.vibeDescription ?? "Auto-detected from the pasted track.",
    };
  }, [suggestions]);
  ...
  const resetFieldToDefault = useCallback((field: string) => {
    if (field === "energy") setEnergyLevel("auto");
    else if (field === "era") setEra("auto");
    else if (field === "tempo") setTempo(null);
    else if (field === "vocals") setVocalGender("auto");
    else if (field === "genres") setSelectedGenres([]);
    else if (field === "moods") setSelectedMoods([]);
    else if (field === "instruments") setSelectedInstruments([]);
    else if (field === "genreNudge") setGenreNudge("");
    else if (field === "bpmTarget") setBpmTarget("");
    else if (field === "chordProgression") setChordProgression("");
    else if (field === "vocalPersona") setVocalPersona("");
    else if (field === "sonicDna") setSonicDna("");
    else if (field === "metaTags") setMetaTagsText("");
    else if (field === "pronunciationGuide") setPronunciationGuide("");
    else if (field === "customExclusions") setCustomExclusions("");
    else if (field === "excludeTags") setExcludeTags([]);
    else if (field === "weirdness") setWeirdness(62);
    else if (field === "styleInfluence") setStyleInfluence(80);
    else if (field === "audioInfluence") setAudioInfluence(25);
  }, []);

  const clearAllAutoFilledFields = useCallback((fields = autoFilledFieldsRef.current) => {
    fields.forEach((field) => resetFieldToDefault(field));
  ...
  const resetAutoFill = (field: string) => {
    ...
    else if (field === "genres") setSelectedGenres(val.split(",").map((part) => part.trim()).filter(Boolean).slice(0, MAX_GENRES));
    else if (field === "moods") setSelectedMoods(val.split(",").map((part) => part.trim()).filter(Boolean).slice(0, MAX_MOODS));
    else if (field === "instruments") setSelectedInstruments(val.split(",").map((part) => part.trim()).filter(Boolean).slice(0, MAX_INSTRUMENTS));
    else if (field === "genreNudge") setGenreNudge(val);
    else if (field === "bpmTarget") setBpmTarget(val);
    else if (field === "chordProgression") setChordProgression(val);
    else if (field === "vocalPersona") setVocalPersona(val);
    else if (field === "sonicDna") setSonicDna(val);
    else if (field === "metaTags") setMetaTagsText(val);
    else if (field === "pronunciationGuide") setPronunciationGuide(val);
    else if (field === "customExclusions") setCustomExclusions(val);
    else if (field === "excludeTags") setExcludeTags(val.split(",").map((part) => part.trim()).filter(Boolean).slice(0, 5));
    else if (field === "weirdness") setWeirdness(Number(val));
    else if (field === "styleInfluence") setStyleInfluence(Number(val));
    else if (field === "audioInfluence") setAudioInfluence(Number(val));
    setAutoFilledFields((prev) => {
      const next = new Set(prev);
      next.add(field);
      autoFilledFieldsRef.current = next;
  ...
  const applySuggestedControls = useCallback((raw: unknown) => {
      // Parse and validate the raw response. Every field is coerced to a safe
      // shape or nulled out — downstream code can trust .length/.slice/.join.
      const data = parseAutofillResponse(raw);

      // Bail if nothing usable came back. This protects against upstream
      // errors where the AI returned a 200 with an empty/garbage body.
      const hasAny =
        data.genres.length > 0 || !!data.era || !!data.energy || !!data.tempo || !!data.vocals ||
        data.moods.length > 0 || data.instruments.length > 0 || !!data.genreNudge ||
        data.bpm !== null || !!data.chordProgression || !!data.vocalPersona || !!data.sonicDna ||
        data.metaTags.length > 0 || !!data.pronunciationGuide || data.negativeHints.length > 0 ||
        data.weirdness !== null || data.styleInfluence !== null || data.audioInfluence !== null;
      if (!hasAny) return;
      ...
      if (data.genres.length > 0) {
        const genres = data.genres.slice(0, MAX_GENRES);
        setSelectedGenres(genres);
        autoFilled.add("genres");
        savedValues["genres"] = genres.join(",");
      }
      if (data.era) {
        setEra(data.era as typeof era);
        autoFilled.add("era");
        savedValues["era"] = data.era;
      }
      if (data.energy) {
        setEnergyLevel(data.energy as typeof energyLevel);
        autoFilled.add("energy");
        savedValues["energy"] = data.energy;
      }
      if (data.tempo) {
        setTempo(data.tempo as typeof tempo);
        autoFilled.add("tempo");
        savedValues["tempo"] = data.tempo;
      }
      if (data.vocals) {
        setVocalGender(data.vocals as typeof vocalGender);
        autoFilled.add("vocals");
        savedValues["vocals"] = data.vocals;
      }
      if (data.moods.length > 0) {
        const moods = data.moods.slice(0, MAX_MOODS);
        setSelectedMoods(moods);
        autoFilled.add("moods");
        savedValues["moods"] = moods.join(",");
      }
      if (data.instruments.length > 0) {
        const instruments = data.instruments.slice(0, MAX_INSTRUMENTS);
        setSelectedInstruments(instruments);
        autoFilled.add("instruments");
        savedValues["instruments"] = instruments.join(",");
      }
      if (data.genreNudge) {
        setGenreNudge(data.genreNudge);
        autoFilled.add("genreNudge");
        savedValues["genreNudge"] = data.genreNudge;
      }

      // ── Suno Lab tab ──────────────────────────────────────────────────────
      if (data.bpm !== null) {
        const bpmStr = String(data.bpm);
        setBpmTarget(bpmStr);
        autoFilled.add("bpmTarget");
        savedValues["bpmTarget"] = bpmStr;
      }
      if (data.chordProgression) {
        setChordProgression(data.chordProgression);
        autoFilled.add("chordProgression");
        savedValues["chordProgression"] = data.chordProgression;
      }
      if (data.vocalPersona) {
        setVocalPersona(data.vocalPersona);
        autoFilled.add("vocalPersona");
        savedValues["vocalPersona"] = data.vocalPersona;
      }
      if (data.sonicDna) {
        setSonicDna(data.sonicDna);
        autoFilled.add("sonicDna");
        savedValues["sonicDna"] = data.sonicDna;
      }
      if (data.metaTags.length > 0) {
        const joined = data.metaTags.join(",");
        setMetaTagsText(joined);
        autoFilled.add("metaTags");
        savedValues["metaTags"] = joined;
      }
      if (data.pronunciationGuide) {
        setPronunciationGuide(data.pronunciationGuide);
        autoFilled.add("pronunciationGuide");
        savedValues["pronunciationGuide"] = data.pronunciationGuide;
      }

      // Negative hints split across chips (known tags) and freeform (custom box)
      if (data.negativeHints.length > 0) {
        const matchedExcludeTags = data.negativeHints.filter((h) => KNOWN_EXCLUSION_VALUES.has(h));
        const customNegativeHints = data.negativeHints.filter((h) => !KNOWN_EXCLUSION_VALUES.has(h));
        if (matchedExcludeTags.length > 0) {
          // Use functional setState to avoid stale-closure issue on excludeTags.
          // This also removes excludeTags from useCallback deps → fewer re-fires.
          setExcludeTags((prev) => {
            const merged = [...new Set([...prev, ...matchedExcludeTags])].slice(0, 5);
            savedValues["excludeTags"] = merged.join(",");
            return merged;
          });
          autoFilled.add("excludeTags");
        }
        if (customNegativeHints.length > 0) {
          const joined = customNegativeHints.join(", ");
          setCustomExclusions(joined);
          autoFilled.add("customExclusions");
          savedValues["customExclusions"] = joined;
        }
      }

      if (data.weirdness !== null) {
        setWeirdness(data.weirdness);
        autoFilled.add("weirdness");
        savedValues["weirdness"] = String(data.weirdness);
      }
      if (data.styleInfluence !== null) {
        setStyleInfluence(data.styleInfluence);
        autoFilled.add("styleInfluence");
        savedValues["styleInfluence"] = String(data.styleInfluence);
      }
      if (data.audioInfluence !== null) {
        setAudioInfluence(data.audioInfluence);
        autoFilled.add("audioInfluence");
        savedValues["audioInfluence"] = String(data.audioInfluence);
      }

      // ── Panel visibility ──────────────────────────────────────────────────
      // Always show Style Controls whenever autofill produced anything — the
      // user's primary signal that "something happened".
      setShowStyleControls(true);

      // Open Suno Lab if any Lab-specific field got autofilled.
      const sunoLabHit =
        data.bpm !== null || !!data.chordProgression || !!data.vocalPersona ||
        !!data.sonicDna || data.metaTags.length > 0 || !!data.pronunciationGuide ||
        data.weirdness !== null || data.styleInfluence !== null || data.audioInfluence !== null;
      if (sunoLabHit) setShowSunoLab(true);

      if (data.negativeHints.length > 0) setShowNegBuilder(true);

      autoFilledFieldsRef.current = autoFilled;
      setAutoFilledFields(autoFilled);
      setAutoFillValues((prev) => ({ ...prev, ...savedValues }));
  }, []); // no deps — all reads use functional setState or are setters (stable)
  ...
  const fetchVideoPreview = useCallback(async (url: string) => {
    ...
    setShowStyleControls(true);
    try {
      const resp = await fetch(`/api/autofill-from-url?url=${encodeURIComponent(url)}`);
      if (resp.ok) {
        const data = await resp.json() as Record<string, unknown> & { preview?: VideoPreview & { cleanTitle?: string } };
        const preview = data.preview;
        if (preview) {
          // For Deezer, the thumbnail comes from the API response; for YouTube use CDN thumb
          const thumb = isDeezerUrl(url) ? (preview.thumbnail ?? null) : (thumbPlaceholder ?? preview.thumbnail ?? null);
          setVideoPreview({ ...preview, thumbnail: thumb });
        }
        applySuggestedControls(data);
        const artist = typeof data.artist === "string" ? data.artist : "";
        const title = typeof data.songTitle === "string" ? data.songTitle : "";
        if (artist && title) {
    ...
    // applySuggestedControls is stable (no deps), but listing it appeases the linter
    // and makes intent explicit. `form` is a react-hook-form instance — stable.
  }, [form, applySuggestedControls]);
  ...
  }, [urlValue, fetchVideoPreview, clearAllAutoFilledFields]);
                ...
                <div className="bg-card border border-primary/15 p-4 space-y-4">
                      ...
                      {MOOD_TAGS.map((mood) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              if (!isDisabled) {
                                setSelectedMoods((p) => toggleSet(p, mood, MAX_MOODS));
                                clearAutoFill("moods");
                              }
                            }}
                            className={cn(
                  ...
                  <div className="space-y-1.5">
                    <label className="flex items-center gap-1.5 text-xs font-semibold text-zinc-400 uppercase tracking-wider">
                      ...
                      {autoFilledFields.has("genreNudge") && <AutoBadge />}
                      {!autoFilledFields.has("genreNudge") && autoFillValues["genreNudge"] && (
                        <ResetAutoFillButton value="restore" onClick={() => resetAutoFill("genreNudge")} />
                      )}
                    </label>
                    <input
                      value={genreNudge}
                      onChange={(e) => { setGenreNudge(e.target.value); clearAutoFill("genreNudge"); }}
                  ...
                  <MoodBoard
                    ...
                    externalResult={moodBoardResult}
              ...
              >
                <div className="bg-card border border-primary/15 p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="flex items-center gap-1.5 font-mono text-[10px] text-zinc-500 uppercase tracking-wider">
                        BPM Anchor
                        {autoFilledFields.has("bpmTarget") && <AutoBadge />}
                        {!autoFilledFields.has("bpmTarget") && autoFillValues["bpmTarget"] && (
                          <ResetAutoFillButton value={autoFillValues["bpmTarget"]} onClick={() => resetAutoFill("bpmTarget")} />
                        )}
                      </label>
                      <input
                        value={bpmTarget}
                        onChange={(e) => { setBpmTarget(e.target.value.replace(/[^\d]/g, "").slice(0, 3)); clearAutoFill("bpmTarget"); }}
                        placeholder="118"
                    ...
                    <div className="space-y-1.5">
                      <label className="flex items-center gap-1.5 font-mono text-[10px] text-zinc-500 uppercase tracking-wider">
                        Chord Progression
                        {autoFilledFields.has("chordProgression") && <AutoBadge />}
                        {!autoFilledFields.has("chordProgression") && autoFillValues["chordProgression"] && (
                          <ResetAutoFillButton value="restore" onClick={() => resetAutoFill("chordProgression")} />
                        )}
                      </label>
                      <input
                        value={chordProgression}
                        onChange={(e) => { setChordProgression(e.target.value); clearAutoFill("chordProgression"); }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="flex items-center gap-1.5 font-mono text-[10px] text-zinc-500 uppercase tracking-wider">
                        Vocal Persona
                        {autoFilledFields.has("vocalPersona") && <AutoBadge />}
                        {!autoFilledFields.has("vocalPersona") && autoFillValues["vocalPersona"] && (
                          <ResetAutoFillButton value="restore" onClick={() => resetAutoFill("vocalPersona")} />
                        )}
                      </label>
                      <textarea
                        value={vocalPersona}
                        onChange={(e) => { setVocalPersona(e.target.value); clearAutoFill("vocalPersona"); }}
                        rows={3}
                    ...
                    <div className="space-y-1.5">
                      <label className="flex items-center gap-1.5 font-mono text-[10px] text-zinc-500 uppercase tracking-wider">
                        Sonic DNA
                        {autoFilledFields.has("sonicDna") && <AutoBadge />}
                        {!autoFilledFields.has("sonicDna") && autoFillValues["sonicDna"] && (
                          <ResetAutoFillButton value="restore" onClick={() => resetAutoFill("sonicDna")} />
                        )}
                      </label>
                      <textarea
                        value={sonicDna}
                        onChange={(e) => { setSonicDna(e.target.value); clearAutoFill("sonicDna"); }}
                        rows={3}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="flex items-center gap-1.5 font-mono text-[10px] text-zinc-500 uppercase tracking-wider">
                        Meta-Tags
                        {autoFilledFields.has("metaTags") && <AutoBadge />}
                        {!autoFilledFields.has("metaTags") && autoFillValues["metaTags"] && (
                          <ResetAutoFillButton value="restore" onClick={() => resetAutoFill("metaTags")} />
                        )}
                      </label>
                      <input
                        value={metaTagsText}
                        onChange={(e) => { setMetaTagsText(e.target.value); clearAutoFill("metaTags"); }}
                    ...
                    <div className="space-y-1.5">
                      <label className="flex items-center gap-1.5 font-mono text-[10px] text-zinc-500 uppercase tracking-wider">
                        Pronunciation Guide
                        {autoFilledFields.has("pronunciationGuide") && <AutoBadge />}
                        {!autoFilledFields.has("pronunciationGuide") && autoFillValues["pronunciationGuide"] && (
                          <ResetAutoFillButton value="restore" onClick={() => resetAutoFill("pronunciationGuide")} />
                        )}
                      </label>
                      <input
                        value={pronunciationGuide}
                        onChange={(e) => { setPronunciationGuide(e.target.value); clearAutoFill("pronunciationGuide"); }}
                    ...
                    <div className="space-y-1.5">
                      <label className="flex items-center justify-between font-mono text-[10px] text-zinc-500 uppercase tracking-wider">
                        <span className="flex items-center gap-1.5">
                          <span>Weirdness</span>
                          {autoFilledFields.has("weirdness") && <AutoBadge />}
                          {!autoFilledFields.has("weirdness") && autoFillValues["weirdness"] && (
                            <ResetAutoFillButton value={autoFillValues["weirdness"]} onClick={() => resetAutoFill("weirdness")} />
                          )}
                        </span>
                        <span>{weirdness}%</span>
                      </label>
                      <input type="range" min={0} max={100} value={weirdness} onChange={(e) => { setWeirdness(Number(e.target.value)); clearAutoFill("weirdness"); }} className="w-full" />
                    </div>
                    <div className="space-y-1.5">
                      <label className="flex items-center justify-between font-mono text-[10px] text-zinc-500 uppercase tracking-wider">
                        <span className="flex items-center gap-1.5">
                          <span>Style Influence</span>
                          {autoFilledFields.has("styleInfluence") && <AutoBadge />}
                          {!autoFilledFields.has("styleInfluence") && autoFillValues["styleInfluence"] && (
                            <ResetAutoFillButton value={autoFillValues["styleInfluence"]} onClick={() => resetAutoFill("styleInfluence")} />
                          )}
                        </span>
                        <span>{styleInfluence}%</span>
                      </label>
                      <input type="range" min={0} max={100} value={styleInfluence} onChange={(e) => { setStyleInfluence(Number(e.target.value)); clearAutoFill("styleInfluence"); }} className="w-full" />
                    </div>
                    <div className="space-y-1.5">
                      <label className="flex items-center justify-between font-mono text-[10px] text-zinc-500 uppercase tracking-wider">
                        <span className="flex items-center gap-1.5">
                          <span>Audio Influence</span>
                          {autoFilledFields.has("audioInfluence") && <AutoBadge />}
                          {!autoFilledFields.has("audioInfluence") && autoFillValues["audioInfluence"] && (
                            <ResetAutoFillButton value={autoFillValues["audioInfluence"]} onClick={() => resetAutoFill("audioInfluence")} />
                          )}
                        </span>
                        <span>{audioInfluence}%</span>
                      </label>
                      <input type="range" min={0} max={100} value={audioInfluence} onChange={(e) => { setAudioInfluence(Number(e.target.value)); clearAutoFill("audioInfluence"); }} className="w-full" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                    <button
                      ...
                      onClick={() => {
                        setWeirdness(25);
                        setStyleInfluence(80);
                        setAudioInfluence(85);
                        clearAutoFill("weirdness");
                        clearAutoFill("styleInfluence");
                        clearAutoFill("audioInfluence");
                      }}
            ...
            {showNegBuilder && (
              ...
              >
                <div className="bg-card border border-primary/15 p-5 space-y-5">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-widest text-zinc-500">
                        <span>Quality & Vibe</span>
                        {autoFilledFields.has("excludeTags") && <AutoBadge />}
                        {!autoFilledFields.has("excludeTags") && autoFillValues["excludeTags"] && (
                          <ResetAutoFillButton value="restore" onClick={() => resetAutoFill("excludeTags")} />
                        )}
                      </p>
                      {excludeTags.some((t) => QUALITY_EXCLUSIONS.map((q) => q.value).includes(t)) && (
                        <button type="button" onClick={() => { setExcludeTags((p) => p.filter((t) => !QUALITY_EXCLUSIONS.map((q) => q.value).includes(t))); clearAutoFill("excludeTags"); }} className="text-[11px] text-zinc-500 hover:text-destructive transition-colors">Clear</button>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                      {QUALITY_EXCLUSIONS.map((preset) => {
                        ...
                        return (
                          <button
                            key={preset.value}
                            type="button"
                            onClick={() => { setExcludeTags((prev) => isChecked ? prev.filter((t) => t !== preset.value) : [...prev, preset.value]); clearAutoFill("excludeTags"); }}
                            className={cn(
                              "px-2.5 py-0.5 font-mono text-[11px] border transition-all",
                              isChecked
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      {ELEMENT_EXCLUSIONS.some((e) => e.value.split(",").some((v) => excludeTags.includes(v))) && (
                        <button type="button" onClick={() => { setExcludeTags((p) => p.filter((t) => !ELEMENT_EXCLUSIONS.flatMap((e) => e.value.split(",")).includes(t))); clearAutoFill("excludeTags"); }} className="text-[11px] text-zinc-500 hover:text-destructive transition-colors">Clear</button>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-1.5">
                  ...
                  <div className="space-y-1.5">
                    <p className="flex items-center gap-1.5 font-mono text-[10px] text-zinc-600 uppercase tracking-widest">
                      <span>Custom terms</span>
                      {autoFilledFields.has("customExclusions") && <AutoBadge />}
                      {!autoFilledFields.has("customExclusions") && autoFillValues["customExclusions"] && (
                        <ResetAutoFillButton value="restore" onClick={() => resetAutoFill("customExclusions")} />
                      )}
                    </p>
                    <input
                      type="text"
                      value={customExclusions}
                      onChange={(e) => { setCustomExclusions(e.target.value); clearAutoFill("customExclusions"); }}
