...
import { SongDnaChart } from "@/components/SongDnaChart";
...
interface TemplateResultProps {
  ...
  onMultiTrack?: () => void;
  onTransition?: () => void;
  onRestoreVersion?: (t: { songTitle: string; styleOfMusic: string; lyrics: string; negativePrompt: string }) => void;
}
    ...
    >
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          {!compact && onMultiTrack && (
            <button
              onClick={onMultiTrack}
      ...
      {/* Prompt quality optimizer — non-compact only */}
      {!compact && (
        <motion.div variants={itemVariants}>
          <PromptOptimizer template={template} />
        </motion.div>
      )}

      {/* Theory tooltips — non-compact + metadata available */}
      {!compact && (metadata?.bpm || metadata?.key || metadata?.energy) && (
        <motion.div variants={itemVariants} className="bg-card border border-primary/15 px-5 py-4">
          <TheoryTooltips
            bpm={metadata.bpm}
      ...
      {/* Lyrics analyzer — non-compact only */}
      {!compact && (
        <motion.div variants={itemVariants}>
          <LyricsAnalyzer lyrics={template.lyrics} />
        </motion.div>
      )}

      {/* Song DNA chart — non-compact only */}
      {!compact && (
        <motion.div variants={itemVariants}>
          <SongDnaChart
            template={template}
            bpm={metadata?.bpm ?? undefined}
            musicalKey={metadata?.key ?? undefined}
      ...
      {/* Version control — non-compact only */}
      {!compact && onRestoreVersion && (
        <motion.div variants={itemVariants}>
          <TemplateVersionControl
            currentTemplate={template}
            onRestoreSnapshot={onRestoreVersion}
          />
        </motion.div>
      )}
    </motion.div>
