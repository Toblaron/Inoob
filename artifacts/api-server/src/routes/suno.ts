...
import { TTLCache, cacheKey, extractVideoId } from "../lib/cache.js";

const router: IRouter = Router();

// ─── In-memory caches ────────────────────────────────────────────────────────
/** YouTube metadata: 1 hour TTL */
const metadataCache = new TTLCache<VideoMetadata>(60 * 60 * 1000);
/** Preview data: 1 hour TTL */
const previewCache = new TTLCache<{ title: string; cleanTitle: string; author: string; thumbnail: string | null; duration: string | null }>(60 * 60 * 1000);
/** Suggestions: 24 hour TTL (genre/era won't change) */
const suggestCache = new TTLCache<{ genres: string[]; era: string | null; energy: string | null; tempo: string | null; vocals: null; songTitle: string; artist: string; mbTags: string[] }>(24 * 60 * 60 * 1000);
/** Generated templates: 24 hour TTL per param hash */
const templateCache = new TTLCache<ReturnType<typeof GenerateSunoTemplateResponse.parse>>(24 * 60 * 60 * 1000);
...
router.post("/generate-template", async (req, res) => {
  try {
    ...
    try {
      ...
      const cached = videoId ? metadataCache.get(videoId) : null;
      if (cached) {
        console.log(`[cache] metadata HIT for ${videoId}`);
        metadata = cached;
      } else {
        metadata = await fetchYouTubeMetadata(youtubeUrl);
        if (videoId) metadataCache.set(videoId, metadata);
      }
    } catch (fetchErr) {
    ...
    // Build a deterministic cache key from all generation parameters (excluding feedbackContext which changes)
    const genParamKey = cacheKey({
      videoId: extractVideoId(youtubeUrl) ?? youtubeUrl,
      vocalGender, energyLevel, era, genreNudge,
      genres: genres?.sort(), moods: moods?.sort(), instruments: instruments?.sort(),
      mode, tempo, excludeTags: excludeTags?.sort(),
      variationIndex: variationIndex ?? 0,
      isInstrumental: isInstrumental ?? false,
      hasManualLyrics: !!(manualLyrics && manualLyrics.trim().length > 20),
    });
    const cachedTemplate = !manualLyrics ? templateCache.get(genParamKey) : null;
    if (cachedTemplate) {
      console.log(`[cache] template HIT for ${genParamKey}`);
      res.json(cachedTemplate);
      return;
    }
...
${context}`;
    ...
    templateCache.set(genParamKey, template);
    res.json(template);
  } catch (err: unknown) {
...
router.get("/suggest", async (req, res) => {
  ...
  try {
    const suggestCacheKey = `${artist.toLowerCase()}::${title.toLowerCase()}`;
    const cachedSuggest = suggestCache.get(suggestCacheKey);
    if (cachedSuggest) {
      console.log(`[cache] suggest HIT for "${artist} - ${title}"`);
...
- "tempo": one of: ballad, slow, mid, groove, uptempo, fast, hyper`,
    ...
    const suggestResult = { genres, era, energy, tempo, vocals: null, songTitle: title, artist, mbTags };
    suggestCache.set(suggestCacheKey, suggestResult);
    res.json(suggestResult);
  } catch (err) {
...
router.get("/youtube-preview", async (req, res) => {
  ...
  try {
    ...
    const cachedPreview = vidId ? previewCache.get(vidId) : null;
    if (cachedPreview) {
      console.log(`[cache] preview HIT for ${vidId}`);
      res.json(cachedPreview);
      return;
    }
    ...
    if (vidId) previewCache.set(vidId, previewResult);
...
/**
 * POST /api/batch
 * Process multiple YouTube URLs in parallel and return templates for each.
 * Body: { urls: string[], options?: Omit<GenerateSunoTemplateBody, "youtubeUrl"> }
 * Response: Array<{ url: string; template: SunoTemplate | null; error?: string }>
 */
router.post("/batch", async (req, res) => {
  const { urls, options = {} } = req.body as { urls: unknown; options?: Record<string, unknown> };

  if (!Array.isArray(urls) || urls.length === 0) {
    res.status(400).json({ error: "urls must be a non-empty array of YouTube URLs" });
    return;
  }
  if (urls.length > 10) {
  ...
  const validUrls = urls.filter((u): u is string => typeof u === "string" && isValidYouTubeUrl(u));
  if (validUrls.length === 0) {
    res.status(400).json({ error: "No valid YouTube URLs provided" });
    return;
  }
  ...
  const results = await Promise.all(
    validUrls.map(async (url) => {
      try {
        const parsed = GenerateSunoTemplateBody.safeParse({ youtubeUrl: url, ...options });
        if (!parsed.success) return { url, template: null, error: "Invalid options" };

        const { youtubeUrl, manualLyrics, vocalGender, energyLevel, era, genreNudge, genres, moods, instruments, mode, tempo, excludeTags, variationIndex, feedbackContext, isInstrumental } = parsed.data;

        const videoId = extractVideoId(youtubeUrl);
        let metadata: VideoMetadata;
        const cachedMeta = videoId ? metadataCache.get(videoId) : null;
        if (cachedMeta) {
          metadata = cachedMeta;
        } else {
          metadata = await fetchYouTubeMetadata(youtubeUrl);
        ...
        const lyricsInstruction =
          metadata.lyricsSource === "user-override"
            ? "⚠️ USER-PROVIDED LYRICS OVERRIDE ACTIVE: Use every lyric line exactly as written."
            : metadata.lyricsSource === "api"
              ? "AUTHENTIC LYRICS from a professional database are provided — use them VERBATIM."
              : metadata.lyricsSource === "captions"
                ? "YouTube captions (approximate) are provided — clean them up and structure with Suno metatags."
                : "No lyrics source available — use your knowledge of this song.";

        const modeInstruction = mode === "cover"
          ? "\n\nGENERATION MODE: AI Cover — Reconstruct this song as faithfully as Suno allows."
          : mode === "inspired"
            ? "\n\nGENERATION MODE: Inspired By — Reimagine boldly. Keep the emotional core, change everything else."
            : "";

        const instrumentalInstruction = isInstrumental
          ? "\n\n🎵 INSTRUMENTAL MODE ACTIVE: No lyrics, only structural/arrangement tags."
          : "";
        ...
        const content = completion.choices[0]?.message?.content;
        if (!content) return { url, template: null, error: "AI returned empty response" };

        const aiResult = JSON.parse(content) as { styleOfMusic: string; title: string; lyrics: string; negativePrompt: string };
        const template = GenerateSunoTemplateResponse.parse({
          songTitle: metadata.title,
          artist: metadata.cleanArtist || metadata.author,
          styleOfMusic: trimStylePrompt(aiResult.styleOfMusic, 900),
        ...
        return { url, template: null, error: (err as Error).message?.slice(0, 120) };
      }
    })
  );
 ...
 * Generate 4 complementary Suno templates for a full arrangement:
...
router.post("/multi-track", async (req, res) => {
  ...
  if (!parsed.success || !isValidYouTubeUrl(parsed.data.youtubeUrl)) {
    res.status(400).json({ error: "Valid youtubeUrl is required" });
    return;
  }

  const { youtubeUrl, vocalGender, energyLevel, era, genreNudge, genres, moods, instruments, mode, tempo, excludeTags, feedbackContext } = parsed.data;
  ...
  try {
    ...
    const context = buildPromptContext(metadata);
    const baseControls = buildStyleControls({ vocalGender, energyLevel, era, genreNudge, genres, moods, instruments, tempo, excludeTags, feedbackContext });

    const trackDefinitions = [
      {
        id: "lead",
        label: "Lead Vocal Track",
        instruction: "Generate the PRIMARY LEAD VOCAL TRACK. This is the main vocal performance with the full lyrics. Focus on the lead singer's delivery, the main melody, and the core arrangement that supports the voice. The styleOfMusic should emphasize the lead vocal character and primary instruments.",
      },
      {
        id: "harmony",
        label: "Harmony & Backing Vocal Track",
        instruction: "Generate a HARMONY / BACKING VOCAL TRACK to layer BEHIND the main vocal. This track provides: lush backing vocal harmonies, call-and-response ad-libs, stacked harmonies at the octave and fifth, atmospheric vocal pads. The lyrics section should contain mostly [Backing vocals], [Harmony], [Vocal pad] and background ad-lib material rather than lead lyrics. styleOfMusic should emphasise \"tight backing harmonies, lush vocal stack, call-and-response, atmospheric choir pad\".",
      },
      {
        id: "instrumental",
        label: "Instrumental Bed Track",
        instruction: "Generate a FULLY INSTRUMENTAL BED TRACK — no vocals whatsoever. This provides the melodic and harmonic foundation: strings, synth pads, piano counter-melodies, ambient textures. The lyrics section must contain ONLY structural/arrangement direction tags with no sung words. styleOfMusic should lead with instrument articulation: legato strings, arpeggiated synth, atmospheric pad, melodic counter-line. Add \"no vocals, no singing\" to the negative prompt.",
      },
      {
        id: "rhythm",
        label: "Rhythm & Percussion Track",
        instruction: "Generate a RHYTHM / PERCUSSION FOCUSED TRACK that provides the beat and rhythmic energy. Drums, bass, rhythm guitar, percussion — no melodic lead. The lyrics section should be entirely structural cue tags: [Kick pattern], [Snare roll], [Hi-hat 16ths], [Bass groove], [Syncopated rhythm]. styleOfMusic should specify exact kick pattern, snare placement, hi-hat style, bass articulation. Add \"no lead vocals, no melodic instruments\" to negative prompt.",
      },
    ];
    ...
    const results = await Promise.all(
      trackDefinitions.map(async (track) => {
        ...
        const content = completion.choices[0]?.message?.content;
        if (!content) throw new Error(`AI returned empty response for ${track.id}`);

        const aiResult = JSON.parse(content) as { styleOfMusic: string; title: string; lyrics: string; negativePrompt: string };
        const template = GenerateSunoTemplateResponse.parse({
          songTitle: `${metadata.title} [${track.label}]`,
          artist: metadata.cleanArtist || metadata.author,
          styleOfMusic: trimStylePrompt(aiResult.styleOfMusic, 900),
          title: aiResult.title,
          lyrics: trimToCharLimit(aiResult.lyrics, 4999),
        ...
        return { trackId: track.id, label: track.label, template };
      })
    );
    ...
    res.json({
      ...
      artist: metadata.cleanArtist || metadata.author,
      tracks: results,
      arrangementNote: "Layer these 4 tracks in Suno by generating each separately and combining in a DAW or Suno's stem feature. Each track is designed to complement the others in the same key and tempo.",
    });
  } catch (err) {
    console.error("multi-track error:", err);
    res.status(500).json({ error: (err as Error).message || "Failed to generate multi-track arrangement" });
  }
});
...
/**
 * POST /api/transition
 * Generate a transition/bridge template between two songs.
 * Body: { fromUrl: string, toUrl: string, transitionStyle?: "smooth" | "key-change" | "genre-blend" | "breakdown" }
 */
router.post("/transition", async (req, res) => {
  const { fromUrl, toUrl, transitionStyle = "smooth" } = req.body as {
    fromUrl?: string;
    toUrl?: string;
    transitionStyle?: "smooth" | "key-change" | "genre-blend" | "breakdown";
  };

  if (!fromUrl || !toUrl || !isValidYouTubeUrl(fromUrl) || !isValidYouTubeUrl(toUrl)) {
    res.status(400).json({ error: "fromUrl and toUrl must be valid YouTube URLs" });
    return;
  }

  try {
    // Fetch both songs in parallel
    const [fromVid, toVid] = extractVideoId(fromUrl), toVidId = extractVideoId(toUrl);
    const [fromMeta, toMeta] = await Promise.all([
      (fromVid && metadataCache.get(fromVid)) ? Promise.resolve(metadataCache.get(fromVid)!) : fetchYouTubeMetadata(fromUrl).then(m => { if (fromVid) metadataCache.set(fromVid, m); return m; }),
      (toVidId && metadataCache.get(toVidId)) ? Promise.resolve(metadataCache.get(toVidId)!) : fetchYouTubeMetadata(toUrl).then(m => { if (toVidId) metadataCache.set(toVidId, m); return m; }),
    ]);

    const fromContext = buildPromptContext(fromMeta);
    const toContext = buildPromptContext(toMeta);

    const transitionStyleGuide = {
      smooth: "Create a musically SMOOTH TRANSITION: shared key or dominant modulation, gradual BPM match, fade from Song A's energy into Song B's energy. The transition should feel seamless to a listener.",
      "key-change": "Create a DRAMATIC KEY CHANGE TRANSITION: end Song A's key, pause, then attack Song B's key. Include a brief tonal ambiguity moment. Use unexpected harmonic pivot.",
      "genre-blend": "Create a GENRE-BLENDING BRIDGE: the transition section should fuse both songs' genres into a hybrid that feels like a natural connection between both worlds.",
      breakdown: "Create a BREAKDOWN TRANSITION: strip everything down to drums + bass only, then rebuild into Song B. Use tension and release. The break creates breathing space between the two songs.",
    }[transitionStyle];

    const userMessage = `You are generating a TRANSITION TEMPLATE that bridges two songs. This template will be used to create a 30–60 second piece that smoothly connects Song A into Song B.

TRANSITION STYLE: ${transitionStyleGuide}
...
Generate a Suno.ai template for the TRANSITION PIECE ONLY (not Song A or B themselves).
- styleOfMusic: Blend both songs' characteristics — include the bridging harmonic movement, tempo matching, and the key musical features of both songs
- title: A creative name for this transition piece
- lyrics: Mostly instrumental direction tags showing the musical progression from Song A to Song B. Include any lyrical bridges or vocal hooks that work as connective tissue.
- negativePrompt: Exclude elements that would break the blend between both songs`;
    ...
    const aiResult = JSON.parse(content) as { styleOfMusic: string; title: string; lyrics: string; negativePrompt: string };
    const template = GenerateSunoTemplateResponse.parse({
      songTitle: `${fromMeta.cleanTitle} → ${toMeta.cleanTitle}`,
      artist: `${fromMeta.cleanArtist} / ${toMeta.cleanArtist}`,
      styleOfMusic: trimStylePrompt(aiResult.styleOfMusic, 900),
      title: aiResult.title,
      lyrics: trimToCharLimit(aiResult.lyrics, 4999),
      negativePrompt: aiResult.negativePrompt,
      tags: ["transition", transitionStyle],
    });

    res.json({
      template,
      fromSong: { title: fromMeta.cleanTitle, artist: fromMeta.cleanArtist, bpm: fromMeta.audioFeatures?.bpm, key: fromMeta.audioFeatures?.key },
      toSong: { title: toMeta.cleanTitle, artist: toMeta.cleanArtist, bpm: toMeta.audioFeatures?.bpm, key: toMeta.audioFeatures?.key },
      transitionStyle,
    });
  } catch (err) {
    console.error("transition error:", err);
    res.status(500).json({ error: (err as Error).message || "Failed to generate transition template" });
  }
});
...
/**
 * POST /api/optimize
 * Score a generated template against Suno best practices.
...
router.post("/optimize", (req, res) => {
  ...
  if (!template || !template.styleOfMusic || !template.lyrics) {
    res.status(400).json({ error: "template with styleOfMusic and lyrics is required" });
    return;
  }

  const style = template.styleOfMusic ?? "";
  const lyrics = template.lyrics ?? "";
  const negPrompt = template.negativePrompt ?? "";

  const suggestions: string[] = [];
  let score = 100;

  // ── Style of Music checks ──────────────────────────────────────────────────
  const styleLen = style.length;
  if (styleLen < 200) {
    score -= 15;
    suggestions.push(`Style prompt is only ${styleLen} chars — aim for 700–900 chars for maximum Suno control. Add more: chord progression, vocal character, instrument articulation, dynamics.`);
  } else if (styleLen < 500) {
    score -= 8;
    suggestions.push(`Style prompt (${styleLen} chars) has room to grow — target 700–900 chars. Add era-specific production details, dynamic range description, and mix depth.`);
  }
  if (styleLen > 900) {
    score -= 10;
    suggestions.push(`Style prompt exceeds 900 chars (${styleLen}) — Suno truncates at 900. Remove the least essential tags.`);
  }

  const hasBPM = /\b\d{2,3}\s*(?:bpm|BPM)\b/.test(style) || /\bBPM:\s*\d/.test(lyrics);
  if (!hasBPM) {
    score -= 8;
    suggestions.push("Missing BPM value — add exact BPM (e.g. \"128 BPM\") to the style prompt for precise tempo control.");
  }

  const hasKey = /\b[A-G][b#]?\s+(?:major|minor|Aeolian|Dorian|Phrygian|Lydian|Mixolydian)\b/i.test(style + " " + lyrics);
  if (!hasKey) {
    score -= 6;
    suggestions.push("No musical key detected — add the song's key (e.g. \"A minor, Aeolian mode\") to the style prompt.");
  }

  const hasChordProg = /\b(?:[A-G][b#]?m?\s*[-–]\s*){2,}/.test(style) || /\b(?:I|ii|iii|IV|V|vi|vii)\s*[-–]/.test(style);
  if (!hasChordProg) {
    score -= 7;
    suggestions.push("No chord progression identified in style — add it (e.g. \"Am-G-C-F loop\" or \"i-VII-VI-V\") for harmonic direction.");
  }

  const hasHookIdentity = /\b(?:hook|anthem|riff|motif|ostinato|vamp|refrain)\b/i.test(style);
  if (!hasHookIdentity) {
    score -= 6;
    suggestions.push("No hook identity stated — open your style prompt with a one-phrase description of the core musical hook (e.g. \"anthemic four-bar rising chorus hook\").");
  }

  const hasDynamics = /\b(?:sparse|dense|crescendo|drop|build|erupts|wall-of-sound|quiet|loud|stripped|full)\b/i.test(style);
  if (!hasDynamics) {
    score -= 5;
    suggestions.push("No dynamic contrast described — add something like \"sparse dry verse erupting into wall-of-sound chorus\" to guide Suno's arrangement.");
  }

  const hasArticulation = /\b(?:staccato|legato|pizzicato|marcato|palm-muted|arpeggiated|tremolo|gliding|sustained)\b/i.test(style + " " + lyrics);
  if (!hasArticulation) {
    score -= 5;
    suggestions.push("No instrument articulation terms — use \"staccato piano fills\", \"legato string swells\", or \"palm-muted rhythm guitar\" instead of plain instrument names.");
  }

  // ── Lyrics / Metadata checks ───────────────────────────────────────────────
  const lyricsLen = lyrics.length;
  if (lyricsLen < 2000) {
    score -= 12;
    suggestions.push(`Lyrics section is only ${lyricsLen} chars (target: 4,900–4,999). Add more: richer section headers with 3+ descriptors, extra [instrument cue] lines, more performance direction parentheticals, ad-lib variants in each chorus.`);
  } else if (lyricsLen < 4000) {
    score -= 6;
    suggestions.push(`Lyrics section at ${lyricsLen} chars — can expand to 4,900. Add more bracketed instrument cues and parenthetical performance directions.`);
  } else if (lyricsLen > 4999) {
    score -= 8;
    suggestions.push(`Lyrics exceed 4,999 chars (${lyricsLen}) — Suno may truncate. Trim the outro or some production cue lines.`);
  }

  const sectionMatches = (lyrics.match(/\[(?:Verse|Chorus|Bridge|Pre-?Chorus|Intro|Outro|Drop|Build|Hook)\b/gi) ?? []);
  if (sectionMatches.length < 4) {
    score -= 6;
    suggestions.push(`Only ${sectionMatches.length} song sections detected — typical Suno templates have 6–10 sections (Intro, Verse 1, Pre-Chorus, Chorus, Verse 2, Bridge, Final Chorus, Outro).`);
  }

  const hasProductionHeader = /\[(?:Produced by|Vocal:|Mix:|Rhythm:|Chord Progression:)/i.test(lyrics);
  if (!hasProductionHeader) {
    score -= 5;
    suggestions.push("Missing production metadata header — add a header block before the first section: [Produced by...], [Vocal:...], [Mix:...], [Rhythm:...], [Key:...], [BPM:...]");
  }

  const hasAdLibs = /\((?:yeah|oh|come on|let's go|hey|woah|uh|ah)\)/i.test(lyrics);
  if (!hasAdLibs) {
    score -= 4;
    suggestions.push("No ad-lib parentheticals found — add (yeah!), (oh-oh), (come on) in chorus sections to humanise the vocal performance.");
  }

  const hasVowelElong = /\w+-[a-z]+-[a-z]+/.test(lyrics) || /\w{2,}-[a-z]{2,}/.test(lyrics);
  if (!hasVowelElong) {
    score -= 3;
    suggestions.push("No vowel elongation markers found — add \"go-o-one\", \"sta-a-ay\" type spellings on emotional peak words.");
  }

  // ── Anti-cliché check ──────────────────────────────────────────────────────
  const clicheWords = ["pulsating", "ethereal tapestry", "sonic journey", "haunting melody", "sonic landscape", "musical tapestry", "immersive experience", "captivating", "mesmerizing", "transcendent"];
  const foundCliches = clicheWords.filter((w) => style.toLowerCase().includes(w) || lyrics.toLowerCase().includes(w));
  if (foundCliches.length > 0) {
    score -= foundCliches.length * 4;
    suggestions.push(`${foundCliches.length} overused AI phrase(s) found: ${foundCliches.map((w) => `"${w}"`).join(", ")}. Replace with specific, production-accurate descriptions.`);
  }

  // ── Negative prompt checks ─────────────────────────────────────────────────
  const negLen = negPrompt.length;
  if (negLen < 120) {
    score -= 5;
    suggestions.push(`Negative prompt is too short (${negLen} chars, target: 180–199). Add more specific exclusions: genres, instruments, production qualities that clash.`);
  } else if (negLen > 199) {
    score -= 5;
    suggestions.push(`Negative prompt exceeds 199 chars (${negLen}). Trim to fit within Suno's limit.`);
  }
  ...
  res.json({
    ...
    grade,
    breakdown: {
      styleLength: styleLen,
      lyricsLength: lyricsLen,
      negativeLength: negLen,
      hasBPM,
      hasKey,
      hasChordProgression: hasChordProg,
      hasHookIdentity,
      hasDynamics,
      hasArticulation,
      hasProductionHeader,
...
/**
 * POST /api/reverse
 * Given an existing Suno template text, reverse-engineer the likely song and settings.
 * Useful when someone shares a Suno creation and you want to recreate or remix it.
 * Body: { templateText: string }
 */
router.post("/reverse", async (req, res) => {
  const { templateText } = req.body as { templateText?: string };

  if (!templateText || templateText.trim().length < 50) {
    res.status(400).json({ error: "templateText must be at least 50 characters" });
    return;
  }
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are a music analyst who reverse-engineers Suno.ai template prompts. Given a Suno style prompt or lyrics block, extract and infer the original song's details and production settings.

Return a JSON object with EXACTLY these fields:
- "inferredSong": object with "title" (string), "artist" (string), "confidence" ("high" | "medium" | "low")
- "genres": array of 1-4 genre strings that match the style
- "era": one of: 50s, 60s, 70s, 80s, 90s, 2000s, 2010s, modern
- "bpm": integer or null
- "key": string (e.g. "A minor") or null
- "energy": one of: very chill, chill, medium, high, intense
- "tempo": one of: ballad, slow, mid, groove, uptempo, fast, hyper
- "vocalGender": one of: male, female, mixed, no vocals
- "moodTags": array of 2-4 mood descriptors
- "instruments": array of 2-5 key instruments mentioned
- "analysisNotes": string (1-2 sentences explaining what this template seems to be for)`,
        },
        {
          role: "user",
          content: `Analyze this Suno template and reverse-engineer its likely source song and settings:\n\n${templateText.slice(0, 3000)}`,
        },
      ],
    });
    ...
    const analysis = JSON.parse(raw) as {
      inferredSong?: { title?: string; artist?: string; confidence?: string };
      genres?: string[];
      era?: string;
      bpm?: number | null;
      key?: string | null;
    ...
    res.json({
      inferredSong: analysis.inferredSong ?? { title: "Unknown", artist: "Unknown", confidence: "low" },
      suggestedSettings: {
        genres: analysis.genres ?? [],
        era: analysis.era ?? null,
        energy: analysis.energy ?? null,
        tempo: analysis.tempo ?? null,
        vocalGender: analysis.vocalGender ?? null,
...
// ─── /api/mood-to-settings ────────────────────────────────────────────────────
/**
 * POST /api/mood-to-settings
 * Convert a free-text mood/vibe description into structured style settings.
 * Body: { moodDescription: string }
 * Example input: "rainy Sunday morning, coffee shop, slightly melancholy but hopeful"
 */
router.post("/mood-to-settings", async (req, res) => {
  const { moodDescription } = req.body as { moodDescription?: string };

  if (!moodDescription || moodDescription.trim().length < 5) {
    res.status(400).json({ error: "moodDescription is required" });
    return;
  }
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You translate mood/vibe descriptions into Suno music generation settings. Given a description of a feeling, place, scene, or vibe, output structured music settings.

Return JSON with EXACTLY these fields:
- "genres": array of 1-3 genre names from: Pop, Indie Pop, Dream Pop, Electropop, Synth-Pop, Rock, Alternative Rock, Indie Rock, Shoegaze, Post-Rock, Hip-Hop, Lo-Fi Hip-Hop, Trap, R&B, Neo-Soul, Soul, Jazz, Smooth Jazz, Nu Jazz, Classical, Orchestral, Cinematic, Ambient, Dark Ambient, IDM, Trip-Hop, Downtempo, Chillhop, Lo-Fi, Electronic, House, Deep House, Techno, Drum & Bass, Dubstep, Future Bass, Synthwave, Vaporwave, Chillwave, Folk, Indie Folk, Country, Blues, Metal, Punk
- "moods": array of 2-4 moods from: Dark, Euphoric, Nostalgic, Melancholic, Aggressive, Romantic, Dreamy, Rebellious, Playful, Mysterious, Cinematic, Hopeful, Angry, Tender, Haunted, Triumphant, Vulnerable, Defiant, Serene, Intense, Wistful, Bittersweet, Groovy, Frantic, Ethereal, Hypnotic, Brooding, Raw, Gritty, Majestic, Eerie, Sensual, Soulful, Cathartic, Blissful, Longing, Psychedelic, Icy, Dusty, Tense, Laid-back, Transcendent, Intimate, Epic
- "energy": one of: very chill, chill, medium, high, intense
- "tempo": one of: ballad, slow, mid, groove, uptempo, fast, hyper
- "era": one of: 50s, 60s, 70s, 80s, 90s, 2000s, 2010s, modern (or null if not implied)
- "instruments": array of 0-3 instruments: Piano, Guitar, Synth, Strings, Bass, Choir, Brass, Drums, Violin, Flute, Organ, Saxophone, Trumpet, Harp, Acoustic Guitar, Electric Guitar, Pad, Rhodes
- "genreNudge": a short descriptive phrase (max 80 chars) that captures the specific vibe, e.g. "lo-fi bedroom recording, cassette warmth, lazy afternoon"
- "reasoning": one sentence explaining the mapping`,
        },
        {
          role: "user",
          content: `Convert this vibe to music settings: "${moodDescription.trim().slice(0, 300)}"`,
        },
      ],
    });
    ...
    res.json({
      ...
      tempo: settings.tempo ?? "mid",
      era: settings.era ?? null,
      instruments: settings.instruments ?? [],
      genreNudge: settings.genreNudge ?? "",
      reasoning: settings.reasoning ?? "",
    });
  } catch (err) {
    console.error("mood-to-settings error:", err);
    res.status(500).json({ error: "Failed to convert mood to settings" });
  }
});
