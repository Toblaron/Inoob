...
// ─── Suno Music Generation via sunoapi.org ────────────────────────────────

const SUNOAPI_BASE = "https://api.sunoapi.org";
const DEFAULT_SUNO_MODEL = "V5_5";

function sunoAuthHeaders(): Record<string, string> {
  const key = process.env.SUNOAPI_KEY;
  if (!key) throw new Error("SUNOAPI_KEY is not configured");
  return { Authorization: `Bearer ${key}`, "Content-Type": "application/json" };
}

/**
 * Maps our internal status string to the canonical SunoTaskStatus.
 * sunoapi.org may return values like "SUCCESS", "PENDING", "FAILED", etc.
 */
function normalizeSunoStatus(raw: string | undefined): "queued" | "processing" | "completed" | "failed" {
...
/**
 * sunoapi.org track shape — present in response.data[].clips[] once complete
 */
interface SunoApiTrack {
  id?: string;
  title?: string;
  audio_url?: string;
  image_url?: string;
...
/**
 * sunoapi.org wraps all responses in { code, msg, data }
 */
...
/**
 * Shape of data.data inside a record-info response
 */
interface SunoApiRecordData {
  taskId?: string;
  status?: string;
  response?: SunoApiTrack[] | SunoApiTrackResponse | null;
  errorCode?: string | null;
  errorMessage?: string | null;
}

/**
 * Some API versions nest tracks under response.sunoData or similar
 */
interface SunoApiTrackResponse {
  sunoData?: SunoApiTrack[];
  clips?: SunoApiTrack[];
}

interface SunoApiCreditsResult {
  credits?: number;
  remaining?: number;
  balance?: number;
  data?: number;
...
/**
 * Extracts a flat list of tracks from various sunoapi.org response shapes.
 * Handles: direct array, { sunoData: [...] }, { clips: [...] }
 */
function extractTracksFromResponse(response: SunoApiTrack[] | SunoApiTrackResponse | null | undefined): SunoApiTrack[] {
  if (!response) return [];
  if (Array.isArray(response)) return response.filter((t) => t && typeof t === "object");
  const nested = (response as SunoApiTrackResponse).sunoData ?? (response as SunoApiTrackResponse).clips ?? [];
  return nested.filter((t) => t && typeof t === "object");
}

/**
 * POST /api/suno/music/generate
 * Submits a music generation request to sunoapi.org and returns the taskId.
 * Body: { styleOfMusic, lyrics, title, negativePrompt?, instrumental?, model? }
 */
router.post("/suno/music/generate", async (req, res) => {
  try {
    if (!process.env.SUNOAPI_KEY) {
      res.status(503).json({ error: "Suno music generation is not configured — SUNOAPI_KEY missing." });
      return;
    }

    const { styleOfMusic, lyrics, title, negativePrompt, instrumental, model } = req.body as {
      styleOfMusic?: string;
      lyrics?: string;
      title?: string;
    ...
    if (!styleOfMusic || !lyrics || !title) {
      res.status(400).json({ error: "styleOfMusic, lyrics, and title are required." });
      return;
    }

    const payload = {
      customMode: true,
      style: styleOfMusic.slice(0, 1000),
      prompt: lyrics.slice(0, 5000),
      title: title.slice(0, 80),
      ...(negativePrompt ? { negativeTags: negativePrompt.slice(0, 200) } : {}),
      instrumental: instrumental === true,
      model: model ?? DEFAULT_SUNO_MODEL,
      callBackUrl: "https://example.com/noop",
    };

    const apiRes = await fetch(`${SUNOAPI_BASE}/api/v1/generate`, {
      method: "POST",
      headers: sunoAuthHeaders(),
      body: JSON.stringify(payload),
    });

    if (!apiRes.ok) {
      const errText = await apiRes.text().catch(() => String(apiRes.status));
      console.error("sunoapi generate error:", apiRes.status, errText);
      res.status(502).json({ error: `Suno API error: ${apiRes.status}` });
      return;
    }

    const envelope = await apiRes.json() as SunoApiEnvelope<{ taskId?: string }>;
    if (envelope.code !== 200 && envelope.code !== undefined) {
      res.status(502).json({ error: envelope.msg ?? `Suno API error: ${envelope.code}` });
      return;
    }

    const taskId = envelope.data?.taskId ?? "";
    if (!taskId) {
      res.status(502).json({ error: "Suno API did not return a task ID." });
      return;
    }
    ...
    res.status(500).json({ error: "Music generation request failed." });
  }
});

/**
 * GET /api/suno/music/status/:taskId
 * Polls sunoapi.org for generation status and returns track data when ready.
 */
router.get("/suno/music/status/:taskId", async (req, res) => {
  try {
    if (!process.env.SUNOAPI_KEY) {
      res.status(503).json({ error: "SUNOAPI_KEY missing." });
      return;
    }
    ...
    if (!apiRes.ok) {
      const errText = await apiRes.text().catch(() => String(apiRes.status));
      console.error("sunoapi status error:", apiRes.status, errText);
      res.status(502).json({ error: `Suno API error: ${apiRes.status}` });
      return;
    }

    const envelope = await apiRes.json() as SunoApiEnvelope<SunoApiRecordData>;
    if (envelope.code !== 200 && envelope.code !== undefined) {
      res.status(502).json({ error: envelope.msg ?? `Suno API error: ${envelope.code}` });
      return;
    }
    ...
    const tracks = extractTracksFromResponse(record?.response);
    const normalizedStatus = normalizeSunoStatus(rawStatus ?? (tracks.length > 0 ? "completed" : "processing"));

    const errMsg = record?.errorMessage ?? undefined;
    ...
    console.error("suno status error:", err);
    res.status(500).json({ error: "Status check failed." });
  }
});

/**
 * GET /api/suno/music/credits
 * Returns remaining sunoapi.org credits for the configured API key.
 */
router.get("/suno/music/credits", async (_req, res) => {
  try {
    if (!process.env.SUNOAPI_KEY) {
      res.status(503).json({ error: "SUNOAPI_KEY missing." });
      return;
    ...
    const apiRes = await fetch(`${SUNOAPI_BASE}/api/v1/generate/credit`, {
      headers: sunoAuthHeaders(),
    });

    if (!apiRes.ok) {
      const errText = await apiRes.text().catch(() => String(apiRes.status));
      console.error("sunoapi credits error:", apiRes.status, errText);
      res.status(502).json({ error: `Suno API error: ${apiRes.status}` });
      return;
    }

    const body = await apiRes.json() as SunoApiCreditsResult;
    const credits = typeof body.data === "number"
      ? body.data
      : (body.credits ?? body.remaining ?? body.balance ?? 0);
    res.json({ credits });
  } catch (err) {
    console.error("suno credits error:", err);
