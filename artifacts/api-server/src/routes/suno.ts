...
async function buildSuggestionPayload(params: {
  title: string;
  artist: string;
  durationSeconds?: number | null;
  musicBrainz?: MusicBrainzData;
  ...
  const { title, artist, durationSeconds = null, musicBrainz, descriptionData } = params;
  const mbData = musicBrainz ?? await Promise.race([
    fetchMusicBrainzData(artist, title, durationSeconds ?? undefined),
    new Promise<MusicBrainzData>((resolve) => setTimeout(() => resolve({}), 7000)),
  ]);
  const aiSuggestion = await fetchAiSongSuggestion(title, artist);
  ...
  const genreNudge = typeof aiSuggestion.genreNudge === "string" && aiSuggestion.genreNudge.trim()
    ...
    : null;
  const parsedDescriptionBpm = descriptionData?.bpm ? Number.parseInt(descriptionData.bpm, 10) : NaN;
  const bpm = typeof aiSuggestion.bpm === "number" && aiSuggestion.bpm >= 40 && aiSuggestion.bpm <= 300
    ? aiSuggestion.bpm
    : Number.isFinite(parsedDescriptionBpm) && parsedDescriptionBpm >= 40 && parsedDescriptionBpm <= 300
      ? parsedDescriptionBpm
      : null;
  const key = typeof aiSuggestion.key === "string" ? aiSuggestion.key : descriptionData?.key ?? null;
  ...
  const pronunciationGuide = typeof aiSuggestion.pronunciationGuide === "string" && aiSuggestion.pronunciationGuide.trim()
    ? aiSuggestion.pronunciationGuide.trim().slice(0, 180)
    : null;
  const negativeHints = (aiSuggestion.negativeHints ?? [])
    .filter((hint) => typeof hint === "string" && hint.trim().length > 0)
    .map((hint) => hint.trim().slice(0, 40))
    .slice(0, 5);
  ...
  return {
    ...
    genreNudge, bpm, key, chordProgression, vocalPersona, sonicDna, metaTags, pronunciationGuide, negativeHints,
    vibeDescription,
...
/**
 * GET /api/autofill-from-url?url=...
 * Robust paste-time autofill endpoint that derives suggestions directly from the source URL.
 */
router.get("/autofill-from-url", async (req, res) => {
  const url = (req.query.url as string ?? "").trim();
  if (!url || !isValidSourceUrl(url)) {
    res.status(400).json({ error: "Provide a valid YouTube or Deezer track URL." });
    return;
  }
  ...
  try {
    ...
    const payload = await buildSuggestionPayload({
      ...
      musicBrainz: base.musicBrainz,
      descriptionData: base.descriptionData,
    });
    ...
    res.json({
      ...
      preview: {
        ...
        author: base.cleanArtist || base.author,
        duration: base.duration || null,
        thumbnail: isDeezerUrl(url)
          ? null
          : (videoIdFromUrl(url) ? `https://img.youtube.com/vi/${videoIdFromUrl(url)}/mqdefault.jpg` : null),
      },
    });
  } catch (err) {
    console.error("[autofill-from-url] error:", err);
    res.status(500).json({ error: "Could not build autofill data from URL." });
  }
});
